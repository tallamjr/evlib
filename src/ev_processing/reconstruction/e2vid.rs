...
impl Default for E2VidConfig {
    fn default() -> Self {
        Self {
            ...
            auto_download: true,
            model_url: "https://github.com/uzh-rpg/rpg_e2vid/raw/master/pretrained/E2VID_lightweight.pth".to_string(),
            intensity_scale: 1.0,
...
impl ImageReconstructor {
    ...
    pub fn update_reconstruction(&mut self, event_tensor: &Tensor) -> CandleResult<Tensor> {
        ...
        if (max_val - min_val).abs() < 1e-6 {
            let constant_val = self.intensity_offset as f32;
        ...
        for row in data {
            for val in row {
                ...
                let scaled_val = norm_val * self.intensity_scale as f32 + self.intensity_offset as f32;
                
                // Clamp to [0,1]
                let clamped_val = scaled_val.max(0.0).min(1.0);
        ...
        Tensor::from_vec(normalized_data, (self.image_height, self.image_width), &*DEVICE)
    }
}
...
impl E2Vid {
    ...
    pub fn process_events(&mut self, events: &Events) -> CandleResult<Tensor> {
        ...
        let reconstructor = self.reconstructor.as_mut().expect("Reconstructor not initialized");
...
impl EventsToVoxelGrid {
    pub fn process_events(&mut self, events: &Events) -> CandleResult<Tensor> {
        ...
        let dt = if (t_max - t_min).abs() < 1e-6 { 1.0 } else { t_max - t_min };
