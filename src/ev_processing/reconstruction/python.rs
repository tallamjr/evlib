...
pub fn events_to_video_py(
    ...
    let output = e2vid.process_events(&events)
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("Error processing events: {}", e)))?;
    
    // Convert tensor to numpy array
    let output_vec = output.to_vec2()
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("Error converting output to vector: {}", e)))?;
    ...
    let output_array = frame.into_pyarray(py).reshape(output_shape)
        .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("Error reshaping output array: {}", e)))?;
...
pub fn reconstruct_events_to_frames_py(
    ...
    for i in 0..num_frames {
        ...
        if frame_events.is_empty() {
            ...
            let empty_frame = vec![0.0; height * width].into_pyarray(py).reshape(empty_shape)?;
            frames.append(empty_frame)?;
            continue;
        }
        
        // Process events
        let output = e2vid.process_events(&frame_events)
            .map_err(|e| PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!("Error processing events: {}", e)))?;
