...
/// Slice events into temporal frames based on the provided configuration
///
/// This function creates slices of events according to the FrameConfig parameters.
/// Returns a vector of event slices, where each slice is a vector of events.
fn slice_events(events: &[Event], config: &FrameConfig) -> TensorResult<Vec<Vec<Event>>> {
    config.validate()?;
    ...
    if let Some(time_window) = config.time_window {
        ...
        Err(Box::new(TensorError(
            "No slicing parameter specified".to_string(),
        )))
    }
}
...
fn slice_events_by_time(
    ...
    let start_time = events[0].t;
    let end_time = events[events.len() - 1].t;
    ...
    let step_size = time_window - overlap;
    if step_size <= 0.0 {
        return Err(Box::new(TensorError(
            "Time window overlap cannot be greater than or equal to time window".to_string(),
        )));
    }
    ...
    while current_start < end_time {
        let current_end = current_start + time_window;

        // Collect events in this time window
        let slice: Vec<Event> = events
            .iter()
            .filter(|event| event.t >= current_start && event.t < current_end)
            .cloned()
            .collect();

        // Only add non-empty slices or incomplete slices if requested
        if !slice.is_empty() || (include_incomplete && current_start < end_time) {
            slices.push(slice);
        }

        current_start += step_size;

        // Break if we would go beyond the end and don't include incomplete
        if !include_incomplete && current_start + time_window > end_time {
            break;
        }
    }
...
fn slice_events_by_count(
    ...
    let overlap_events = (overlap as usize).min(event_count);
    let step_size = event_count - overlap_events;

    if step_size == 0 {
        return Err(Box::new(TensorError(
            "Event count overlap cannot be greater than or equal to event count".to_string(),
        )));
    }
    ...
    while start_idx < events.len() {
        let end_idx = (start_idx + event_count).min(events.len());
        let slice = events[start_idx..end_idx].to_vec();

        // Only add if we have the full count or include incomplete is true
        if slice.len() == event_count || (include_incomplete && !slice.is_empty()) {
            slices.push(slice);
        }
...
fn slice_events_by_time_bins(
    ...
    let bin_duration = total_duration / n_time_bins as f64;
    let overlap_duration = bin_duration * overlap;
    let step_size = bin_duration - overlap_duration;

    let mut slices = Vec::new();

    for i in 0..n_time_bins {
        let bin_start = start_time + i as f64 * step_size;
        let bin_end = bin_start + bin_duration;

        let slice: Vec<Event> = events
            .iter()
            .filter(|event| event.t >= bin_start && event.t < bin_end)
            .cloned()
            .collect();
...
fn slice_events_by_event_bins(
    ...
    let bin_size = total_events / n_event_bins;
    let overlap_events = (bin_size as f64 * overlap) as usize;
    let step_size = bin_size - overlap_events;

    let mut slices = Vec::new();

    for i in 0..n_event_bins {
        let start_idx = i * step_size;
        let end_idx = (start_idx + bin_size).min(total_events);
...
pub fn to_timesurface_enhanced(
    ...
    if single_polarity {
        ...
        let polarities: std::collections::HashSet<i8> = events.iter().map(|e| e.polarity).collect();
        if polarities.len() > 1 {
            return Err(Box::new(TensorError(
...
pub fn to_timesurface_enhanced_polars(
    ...
    let df = crate::ev_formats::python::build_polars_dataframe(
        events,
        crate::ev_formats::EventFormat::HDF5,
    )
...
pub fn to_bina_rep_enhanced_polars(
    ...
    .map_err(|e| Box::new(TensorError(format!("Failed to create DataFrame: {}", e))))?;
