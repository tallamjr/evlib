...
//
// One thread per pixel walks the batch of T frames, float32 log intensity
// (evsim_run) or uint8 intensity mapped through a 256-entry log LUT on the
// device (evsim_run_u8, LUT set once with evsim_set_lut). Pass 1 counts
...
__global__ void count_k(long long n, const void *frames, const float *d_lut,
                        ...
                        const long long *tlast, const float *prev, long long prev_t, int width,
                        long long *counts) {
  ...
  counts[i] = walk_pixel<false, KIND>(i, n, frames, s_lut, t_ns, n_frames, cpos[i], cneg[i],
                                      refractory_ns, init, lref[i], tlast[i], prev[i], prev_t,
                                      width, 0, nullptr, nullptr, nullptr, nullptr, nullptr,
                                      nullptr);
}
...
__global__ void write_k(long long n, const void *frames, const float *d_lut,
                        ...
                        float *prev, long long prev_t, int width, const long long *offsets,
  ...
  walk_pixel<true, KIND>(i, n, frames, s_lut, t_ns, n_frames, cpos[i], cneg[i], refractory_ns,
                         init, lref[i], tlast[i], prev[i], prev_t, width, offsets[i], out_x,
                         out_y, out_t, out_p, &lref_new, &tlast_new);
  lref[i] = lref_new;
  tlast[i] = tlast_new;
...
// Shared body of evsim_run (KIND 0, float32) and evsim_run_u8 (KIND 1, uint8).
template <int KIND>
static int run_impl(void *handle, const void *frames, const long long *t_ns, int n_frames,
                    unsigned short *out_x, unsigned short *out_y, long long *out_t,
  ...
  long long frames_len = (long long)n_frames * n * (long long)elem;
  ...
  EVSIM_CHECK(cudaMemcpy(h->d_frames, frames, (size_t)frames_len, cudaMemcpyHostToDevice));
  ...
  count_k<KIND><<<blocks, threads>>>(n, h->d_frames, h->d_lut, h->d_tns, n_frames, h->d_cpos,
                                     h->d_cneg, h->refractory_ns, init, h->d_lref, h->d_tlast,
                                     h->d_prev, h->prev_t, h->width, h->d_counts);
  ...
  *n_events = total;
  ...
  write_k<KIND><<<blocks, threads>>>(n, h->d_frames, h->d_lut, h->d_tns, n_frames, h->d_cpos,
                                     h->d_cneg, h->refractory_ns, init, h->d_lref, h->d_tlast,
                                     h->d_prev, h->prev_t, h->width, h->d_offsets, h->d_out_x,
                                     h->d_out_y, h->d_out_t, h->d_out_p);
  EVSIM_CHECK(cudaGetLastError());
  EVSIM_CHECK(cudaDeviceSynchronize());
...
int evsim_run(void *handle, const float *frames, const long long *t_ns, int n_frames,
  ...
  return run_impl<0>(handle, frames, t_ns, n_frames, out_x, out_y, out_t, out_p, capacity,
                     n_events);
}

// Set the 256-entry float32 log LUT used by evsim_run_u8 (host pointer).
...
int evsim_run_u8(void *handle, const unsigned char *frames, const long long *t_ns, int n_frames,
                 unsigned short *out_x, unsigned short *out_y, long long *out_t,
                 signed char *out_p, long long capacity, long long *n_events) {
  return run_impl<1>(handle, frames, t_ns, n_frames, out_x, out_y, out_t, out_p, capacity,
                     n_events);
}
