...
2. **Lazy and engine-selectable**: loaders return a Polars `LazyFrame`, so the same query runs on the CPU streaming engine or on the GPU via cudf-polars (`collect(engine="gpu")`) where CUDA is available. The GPU path uses cudf-polars' own default memory resource, not CUDA managed memory (UVM); see [Performance: GPU memory footprint](../getting-started/performance.md#gpu-memory-footprint) for measured VRAM use.
3. **Query layer vs compute layer**: Polars is the query, filter, and transform layer (CPU and the cudf GPU engine). The native Rust, CUDA, and Metal scatter-add kernels are the compute layer for the stacked histogram. Polars-GPU is not a free win for a single transfer-bound operation; the custom scatter-add kernels are where the GPU wins.
