        ...
        for memory_limit in &memory_limits {
            ...
            group.bench_with_input(
                ...
                |b, (events, memory_limit)| {
                    b.iter(|| {
                        let chunk_size = PolarsEventStreamer::calculate_optimal_chunk_size(events.len(), *memory_limit);
                        let streamer = PolarsEventStreamer::new(chunk_size, EventFormat::HDF5);
                ...
                |b, (events, chunk_size)| {
                    b.iter(|| {
                        ...
                        let final_df = if dataframes.len() == 1 {
                            ...
                            concat(&lazy_frames, UnionArgs::default()).unwrap().collect().unwrap()
                        };
...
fn benchmark_streaming_configuration(c: &mut Criterion) {
    ...
    let configs = vec![
        ...
        ("high_memory", StreamingConfig {
            chunk_size: 2_000_000,
            memory_limit_mb: 1024,
            progress_interval: 5_000_000,
        }),
        ("low_memory", StreamingConfig {
            chunk_size: 250_000,
            memory_limit_mb: 128,
    ...
    for (config_name, config) in configs {
        ...
        group.bench_with_input(
            ...
            |b, (events, config)| {
                b.iter(|| {
                    let streamer = PolarsEventStreamer::with_config(config.clone(), EventFormat::HDF5);
