...
fn benchmark_event_filtering(c: &mut Criterion) {
    ...
    group.bench_with_input(
        ...
        |b, events| {
            b.iter(|| {
                let filtered: Vec<_> = events.iter().filter(|e| e.t >= 0.1 && e.t <= 0.5).collect();
    ...
    group.bench_with_input(
        ...
        |b, events| {
            b.iter(|| {
                let filtered: Vec<_> = events.iter().filter(|e| e.polarity).collect();
                black_box(filtered.len())
            })
        },
    );
            ...
            |b, (events, chunk_size)| {
                b.iter(|| {
                    ...
                    for chunk in events.chunks(*chunk_size) {
                        // Simulate processing
                        let chunk_sum: u64 = chunk.iter().map(|e| e.x as u64 + e.y as u64).sum();
...
fn benchmark_format_conversion(c: &mut Criterion) {
    ...
    group.bench_with_input(
        ...
        |b, events| {
            b.iter(|| {
                let converted: Vec<_> = events.iter().map(|e| (e.t * 1_000_000.0) as i64).collect();
                black_box(converted.len())
            })
        },
    );
