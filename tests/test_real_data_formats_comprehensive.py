...
class TestRealDataFormats:
    ...
    def validate_events(
        self, x: np.ndarray, y: np.ndarray, t: np.ndarray, p: np.ndarray, file_key: str
    ) -> Dict[str, any]:
    ...
    def test_filtering_functionality(self):
        ...
        print(
            f"Filtering tests: full {len(x_full)}, time {len(x_time)}, spatial {len(x_spatial)}, polarity {len(x_pos)}"
        )
    ...
    def test_memory_efficiency(self):
        ...
        print(
            f"Memory efficiency: {memory_used:.1f}MB for {event_count} events ({bytes_per_event:.1f} bytes/event)"
        )
    ...
    def test_performance_summary(self):
        ...
        for file_key, data in self.results.items():
            ...
            print(
                f"  Coordinates: ({validation['coordinate_bounds']['x_min']},{validation['coordinate_bounds']['y_min']}) to ({validation['coordinate_bounds']['x_max']},{validation['coordinate_bounds']['y_max']})"
            )
            print(f"  Duration: {validation['time_range']['duration']:.3f}s")
            print(
                f"  Polarity: {validation['polarity_distribution']['positive']} pos, {validation['polarity_distribution']['negative']} neg"
            )
