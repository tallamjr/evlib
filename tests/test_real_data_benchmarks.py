...
class RealDataProcessingBenchmarks:
    ...
    def test_numpy_real_data_filtering(self, benchmark, file_key):
        ...
        def filter_events():
            ...
            t_range = t_max - t_min
            temporal_mask = (
                (timestamp >= t_min + 0.25 * t_range) &
                (timestamp <= t_min + 0.75 * t_range)
            )
    ...
    def test_polars_real_data_aggregation(self, benchmark, file_key):
        ...
        def aggregate_events():
            ...
            activity_df = df.with_columns([
                ((pl.col("timestamp") - t_min) / (t_max - t_min) * 100)
                .floor()
                .clip(0, 99)
                .cast(pl.Int32)
                .alias("time_bin")
            ]).group_by("time_bin").agg(pl.count().alias("events_in_bin"))
...
class RealDataMemoryBenchmarks:
    ...
    def test_memory_peak_usage_numpy(self, benchmark, file_key):
        ...
        def memory_peak_test():
            ...
            hist = stacked_histogram(x, y, polarity, timestamp, 5, 
                                   file_info["sensor_resolution"][1], file_info["sensor_resolution"][0])
    ...
    def test_memory_peak_usage_polars(self, benchmark, file_key):
        ...
        def memory_peak_test():
            ...
            hist = stacked_histogram(df, bins=5, 
                                   height=file_info["sensor_resolution"][1], 
                                   width=file_info["sensor_resolution"][0])
            after_hist_memory = psutil.Process().memory_info().rss / 1024 / 1024
