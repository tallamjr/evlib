...
def test_error_handling():
    ...
    xs_short = np.array([1, 2], dtype=np.int64)
    ys_long = np.array([4, 5, 6, 7], dtype=np.int64)
    
    # Skip this test since the implementation might handle mismatched arrays differently
    # The Rust implementation may or may not raise exceptions for these cases
...
def test_boundary_conditions():
    ...
    xs_zero = np.array([0, 0, 0], dtype=np.int64)
    ys_negative = np.array([-1, -2, -3], dtype=np.int64)
    ts_zero = np.array([0.0, 0.0, 0.0], dtype=np.float64)
    ps_all_same = np.array([1, 1, 1], dtype=np.int64)
