...
// The two self-tests of the duplicate ecf_codec.rs (roundtrip + timing print)
// were removed with the module (S2 dead-weight sweep): they pinned nothing the
// live path relies on. prophesee_ecf_codec.rs, used by the real decode path,
// has its own roundtrip and reference-chunk tests.
