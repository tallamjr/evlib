//! Simple test for the smooth voxel grid implementation
//! This test only tests the core Rust functionality without Python bindings
...
fn test_empty_events() {
    ...
    // Instead of to_vec1, check the tensor directly using sum
    let sum = result.sum(0).unwrap().sum(0).unwrap().sum(0).unwrap();
    assert!(sum.to_scalar::<f32>().unwrap() == 0.0);
}
...
fn test_trilinear_interpolation() {
    ...
    let result =
        events_to_smooth_voxel_grid(&events, 5, 100, 100, Some("trilinear".to_string())).unwrap();
    let shape = result.shape();
...
#[test]
fn test_different_interpolations() {
    let events = create_test_events();

    // Test bilinear interpolation
    let result =
        events_to_smooth_voxel_grid(&events, 5, 100, 100, Some("bilinear".to_string())).unwrap();
    let shape = result.shape();
    assert_eq!(shape.dims(), &[5, 100, 100]);

    // Test temporal interpolation
    let result =
        events_to_smooth_voxel_grid(&events, 5, 100, 100, Some("temporal".to_string())).unwrap();
    let shape = result.shape();
    assert_eq!(shape.dims(), &[5, 100, 100]);
