...
try:
    from evlib.polars_utils import (
        load_events_as_polars_dataframe,  # noqa: F401
        events_to_polars_dataframe_with_metadata,  # noqa: F401
...
class BenchmarkData:
    ...
    def create_synthetic_events(
        num_events: int, width: int = 640, height: int = 480, duration: float = 1.0, seed: int = 42
    ...
    def create_polars_dataframe(
        x: np.ndarray, y: np.ndarray, timestamp: np.ndarray, polarity: np.ndarray
    ) -> "pl.DataFrame":
        """Create Polars DataFrame from event arrays."""
        if not POLARS_AVAILABLE:
            pytest.skip("Polars not available")

        return pl.DataFrame({"x": x, "y": y, "timestamp": timestamp, "polarity": polarity})

    @staticmethod
    def save_events_to_file(
        x: np.ndarray, y: np.ndarray, timestamp: np.ndarray, polarity: np.ndarray, filepath: str
    ):
...
class FilteringBenchmarks:
    ...
    def test_polars_spatial_filtering(self, benchmark, size_name):
        ...
        def spatial_filter():
            ...
            filtered_df = self.df.filter(
                (pl.col("x") >= 100) & (pl.col("x") <= 540) & (pl.col("y") >= 50) & (pl.col("y") <= 430)
            )
            memory_used = memory_profiler.get_memory_delta()
    ...
    def test_polars_temporal_filtering(self, benchmark, size_name):
        ...
        def temporal_filter():
            ...
            filtered_df = self.df.filter((pl.col("timestamp") >= t_start) & (pl.col("timestamp") <= t_end))
            memory_used = memory_profiler.get_memory_delta()
            return filtered_df, memory_used
...
class AggregationBenchmarks:
    ...
    def test_numpy_event_counting(self, benchmark, size_name):
        ...
        benchmark.extra_info.update({"dataset_size": size, "memory_mb": memory_used, "operation": "counting"})
    ...
    def test_numpy_spatial_statistics(self, benchmark, size_name):
        ...
        benchmark.extra_info.update(
            {"dataset_size": size, "memory_mb": memory_used, "operation": "spatial_statistics"}
        )
    ...
    def test_numpy_temporal_binning(self, benchmark, size_name):
        ...
        benchmark.extra_info.update(
            {"dataset_size": size, "memory_mb": memory_used, "operation": "temporal_binning"}
        )
...
class RepresentationBenchmarks:
    ...
    def test_numpy_voxel_grid(self, benchmark, size_name):
        ...
        def create_voxel():
            ...
            voxel = create_voxel_grid(
                self.x, self.y, self.timestamp, self.polarity, self.sensor_resolution, num_bins=5
            )
            memory_used = memory_profiler.get_memory_delta()
    ...
    def test_polars_voxel_grid(self, benchmark, size_name):
        ...
        def create_voxel():
            memory_profiler.reset()
            voxel = create_voxel_grid(self.df, sensor_resolution=self.sensor_resolution, num_bins=5)
            memory_used = memory_profiler.get_memory_delta()
            return voxel, memory_used
    ...
    def test_numpy_stacked_histogram(self, benchmark, size_name):
        ...
        def create_histogram():
            ...
            hist = stacked_histogram(
                self.x, self.y, self.polarity, self.timestamp, bins=5, height=480, width=640
            )
            memory_used = memory_profiler.get_memory_delta()
    ...
    def test_polars_stacked_histogram(self, benchmark, size_name):
        ...
        def create_histogram():
            memory_profiler.reset()
            hist = stacked_histogram(self.df, bins=5, height=480, width=640)
            memory_used = memory_profiler.get_memory_delta()
            return hist, memory_used
    ...
    def test_numpy_time_surface(self, benchmark, size_name):
        ...
        def create_surface():
            ...
            surface = create_time_surface(
                self.x, self.y, self.timestamp, self.polarity, self.sensor_resolution
            )
            memory_used = memory_profiler.get_memory_delta()
    ...
    def test_polars_time_surface(self, benchmark, size_name):
        ...
        def create_surface():
            memory_profiler.reset()
            surface = create_time_surface(self.df, sensor_resolution=self.sensor_resolution)
            memory_used = memory_profiler.get_memory_delta()
            return surface, memory_used
    ...
    def test_numpy_event_histogram(self, benchmark, size_name):
        ...
        def create_histogram():
            memory_profiler.reset()
            hist = create_event_histogram(self.x, self.y, self.polarity, self.sensor_resolution)
            memory_used = memory_profiler.get_memory_delta()
            return hist, memory_used
    ...
    def test_polars_event_histogram(self, benchmark, size_name):
        ...
        def create_histogram():
            memory_profiler.reset()
            hist = create_event_histogram(self.df, sensor_resolution=self.sensor_resolution)
            memory_used = memory_profiler.get_memory_delta()
            return hist, memory_used
...
class ScalabilityBenchmarks:
    ...
    def test_numpy_scalability_voxel_grid(self, benchmark, size_name):
        ...
        def create_voxel():
            memory_profiler.reset()
            voxel = create_voxel_grid(x, y, timestamp, polarity, sensor_resolution, num_bins=5)
            memory_used = memory_profiler.get_memory_delta()
            return voxel, memory_used
    ...
    def test_polars_scalability_voxel_grid(self, benchmark, size_name):
        ...
        def create_voxel():
            memory_profiler.reset()
            voxel = create_voxel_grid(df, sensor_resolution=sensor_resolution, num_bins=5)
            memory_used = memory_profiler.get_memory_delta()
            return voxel, memory_used
...
class MemoryUsageBenchmarks:
    ...
    def test_numpy_memory_efficiency(self, benchmark, size_name):
        ...
        def memory_test():
            ...
            # Create voxel grid
            voxel = create_voxel_grid(x, y, timestamp, polarity, (640, 480), num_bins=10)
        ...
        benchmark.extra_info.update({"dataset_size": size, "implementation": "numpy", **result})
    ...
    def test_polars_memory_efficiency(self, benchmark, size_name):
        ...
        def memory_test():
            ...
            # Create voxel grid
            voxel = create_voxel_grid(df, sensor_resolution=(640, 480), num_bins=10)
            peak_after_voxel = memory_profiler.get_memory_delta()
        ...
        benchmark.extra_info.update({"dataset_size": size, "implementation": "polars", **result})
