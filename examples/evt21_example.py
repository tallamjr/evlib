...
def create_synthetic_evt21_file(
    ...
    with open(output_path, "wb") as f:
        ...
        header += f"% date 2025-01-16 12:00:00\n"
        header += f"% source synthetic_generator\n"
        header += f"% end\n"
        ...
        while i < num_events:
            ...
            if np.random.random() < vectorized_ratio and i + 31 < num_events:
                ...
                for j in range(pixels_in_vector):
                    ...
                    if (abs(x_coords[i + j] - base_x) <= 31 and 
                        y_coords[i + j] == base_y and 
                        polarities[i + j] == base_polarity):
                        validity_mask |= (1 << j)
    ...
    print(f"✓ Synthetic EVT2.1 file created successfully")
...
def example_2_basic_loading():
    ...
    try:
        ...
        print(f"  Data types: timestamps={type(timestamps[0])}, x={type(x_coords[0])}, y={type(y_coords[0])}, pol={type(polarities[0])}")
        ...
        for i in range(min(5, len(timestamps))):
            print(f"  Event {i}: t={timestamps[i]:.6f}s, x={x_coords[i]}, y={y_coords[i]}, pol={polarities[i]}")
...
def example_3_vectorized_capabilities():
    ...
    for file_path, vector_ratio, description in test_files:
        ...
        try:
            ...
            print(f"  Compression ratio: {len(timestamps) / (Path(file_path).stat().st_size / 8):.2f} events/word")
...
def example_4_filtering_and_processing():
    ...
    try:
        ...
        print(f"  Spatial range: x=[{x_coords_roi.min()}-{x_coords_roi.max()}], y=[{y_coords_roi.min()}-{y_coords_roi.max()}]")
        ...
        for i in range(min(10, num_windows)):  # Show first 10 windows
            ...
            window_events = evlib.load_events(
                synthetic_file, 
...
def example_6_real_world_usage():
    ...
    try:
        ...
        while current_time < end_time and chunk_count < 10:  # Process first 10 chunks
            ...
            chunk_events = evlib.load_events(
                realistic_file,
